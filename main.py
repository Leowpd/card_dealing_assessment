...
INSTRUCTIONS_ENGLISH = pygame.transform.scale(IMAGE1, (cnst.INSTRUCTIONS_WIDTH, cnst.INSTRUCTIONS_HEIGHT))
INSTRUCTIONS_MAORI = pygame.transform.scale(IMAGE2, (cnst.INSTRUCTIONS_WIDTH, cnst.INSTRUCTIONS_HEIGHT))
CARDS_ERROR_ENGLISH = pygame.transform.scale(IMAGE3, (cnst.CARDS_ERROR_WIDTH, cnst.CARDS_ERROR_HEIGHT))
CARDS_ERROR_MAORI = pygame.transform.scale(IMAGE4, (cnst.CARDS_ERROR_WIDTH, cnst.CARDS_ERROR_HEIGHT))
...
def process_game_file(user_game):
    ...
    for char in range(0, len(deck_file_content), y):
        ...
        card_code_li = []
        x = 1   #this splits the card code every 1 digit (e.g. "AS" goes to ['A', 'S']).
        for char in range(0, len(card_code), x):
            card_code_li.append(card_code[char : char + x])

        list_of_card_codes.append(card_naming_sys[card_code_li[0]] + " of " + card_naming_sys[card_code_li[1]])
...
def accept_inputs():
    ...
    while True:
        user_game = input(cnst.TEXTS[str(user_language)+"_GAME_INPUT_TEXT"]+cnst.GAMES)
...
def draw_window(players, player_names, user_language, user_wildcards_value):
    ...
    for player in player_names:
        ...
        for i in range(len(temp_list)):
            ...
            WINDOW.blit(pygame.transform.scale(pygame.image.load(os.path.join('assets', str(card))), (cnst.CARD_WIDTH, cnst.CARD_HEIGHT)), (x, y))
